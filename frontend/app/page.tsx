...
const nav: { key: NavKey; label: string; icon: typeof Film }[] = [
  ...
  { key: "operations", label: "协作与效率", icon: Activity },
];
...
function Storyboard({ project, shot, setSelected, regenerate, approveKeyframe, busy }: { project: Project; shot: Shot; setSelected:(id:string)=>void; regenerate:()=>void; approveKeyframe:()=>void; busy:boolean }) {
...
function Operations({project}:{project:Project}) {
  ...
  const [logs,setLogs]=useState(project.audit_logs||[]);
  useEffect(()=>{
    Promise.all([
      fetch(`${API}/api/projects/${project.id}/metrics`).then(r=>r.ok?r.json():Promise.reject()),
      fetch(`${API}/api/projects/${project.id}/audit-logs`).then(r=>r.ok?r.json():Promise.reject()),
    ]).then(([metricData,auditData])=>{setMetrics(metricData);setLogs(auditData);}).catch(()=>{});
  },[project.id]);
  const percent=(value:number|null|undefined)=>value==null?"—":`${Math.round(value*100)}%`;
  return <div className="page scroll-page"><SectionHead eyebrow="COLLABORATION & EFFICIENCY" title="协作与效率" copy="把预算、返工、失效依赖与每一次人工决策放进同一条可审计时间线。"><MockBadge visible={project.data_mode==="demo"} compact/><button className="outline-btn"><MessageSquareText size={16}/> 导出审计</button></SectionHead>
    <div className="metrics-row"><Metric icon={ShieldCheck} label="镜头一次通过率" value={percent(metrics?.shot_first_pass_rate)} sub={metrics?.metric_source||"等待运营记录"} color="teal"/><Metric icon={RefreshCw} label="返工率" value={percent(metrics?.rework_rate)} sub={`${metrics?.stale_shot_count||0} 个失效镜头`} color="coral"/><Metric icon={CircleDollarSign} label="单镜平均成本" value={metrics?.average_cost_per_completed_shot==null?"—":money(metrics.average_cost_per_completed_shot)} sub={`预算 ${money(metrics?.budget_limit||project.budget_limit||8)}`} color="yellow"/><Metric icon={TimerReset} label="UNKNOWN 任务" value={String(metrics?.unknown_task_count||0)} sub="需要 Provider 对账" color="blue"/></div>
    <div className="two-column"><div className="panel"><PanelTitle title="业务状态" extra={`Project v${project.version||1}`}/><div className="attention"><Attention color="teal" title={`组织 ${project.organization_id||"org_demo"}`} copy={`Owner · ${project.owner_id||"user_owner"}`}/><Attention color={(metrics?.stale_shot_count||0)>0?"yellow":"teal"} title={`${metrics?.stale_shot_count||0} 个依赖产物失效`} copy="资产新版本会阻断过期镜头继续交付"/><Attention color={(metrics?.unknown_task_count||0)>0?"coral":"teal"} title={`${metrics?.unknown_task_count||0} 个任务状态未知`} copy="UNKNOWN 只允许查询和对账，不会盲目重提"/></div></div>
    <div className="panel"><PanelTitle title="最近审计记录" extra={`${logs.length} 条`}/><div className="attention">{logs.length?logs.slice(-6).reverse().map(log=><Attention key={log.id} color="teal" title={log.action} copy={`${log.actor_id} · ${log.target_type}/${log.target_id}`}/>):<Attention color="yellow" title="暂无可展示记录" copy="连接 API 后会显示真实操作者与目标"/>}</div></div></div>
  </div>;
}
...
function Delivery({project,flash}:{project:Project;flash:(s:string)=>void}) {
  ...
  async function backendAction(action:"quality-run"|"export") {
    try {
      const response = await fetch(`${API}/api/projects/${project.id}/${action}`, {method:"POST"});
