...
export interface Shot {
  ...
  first_frame_asset_id?: string; last_frame_asset_id?: string;
}
...
export interface GenerationRun { id: string; shot_id: string; status: string; progress: number; cost: number; elapsed_sec: number; provider: string; lifecycle_status?: string; media_kind?: string; artifact_id?: string; candidate_id?: string; }
export interface MediaArtifact {
  id: string; shot_id?: string; kind: "keyframe"|"video"|"audio"|"subtitle"|"final_video";
  storage_path: string; mime_type: string; checksum_sha256: string; file_size: number;
  source: string; provider?: string; review_status: "pending"|"approved"|"rejected"|"not_required";
  measured: boolean; metadata: Record<string, unknown>;
}
...
export interface Project {
  ...
  subtitle_cues?: { id:string; shot_id:string; start_sec:number; end_sec:number; text:string; speaker?:string }[];
}
