...
| TTS / 字幕 / FFmpeg 成片 | 接口准备 | 当前环境未配置真实 TTS 和 FFmpeg，尚未形成可播放成片 |
...
- 对接至少一个真实图像 Provider 和一个真实 I2V Provider，建立 2–4 关键帧候选审核。
- 完成 TTS 角色声音版本、句级时间轴、SRT/ASS 字幕与音频轨道。
- 实现 FFmpeg 装配、转码、封面、交付 Manifest 和可下载成片。
